...
test.describe('정리된 퍼널 관리 기능 테스트', () => {
  ...
  test('고급 사용자 행동 분석 섹션 확인', async ({ page }) => {
    ...
    await expect(page.locator('text=평균 세션 시간')).toBeVisible();
    await expect(page.locator('text=바운스율')).toBeVisible();
    await expect(page.locator('text=페이지/세션')).toBeVisible();
  });
  ...
  test('페이지 성능 섹션 확인', async ({ page }) => {
    ...
    // 페이지 성능 섹션 확인
    await expect(page.locator('h3:has-text("페이지 성능")')).toBeVisible();
  ...
  test('스크롤 깊이 분석 섹션 확인', async ({ page }) => {
    ...
    // 스크롤 깊이 데이터 메시지 확인
    await expect(page.locator('text=스크롤 깊이 데이터')).toBeVisible();
    await expect(page.locator('text=실시간 스크롤 깊이 분석 데이터가 여기에 표시됩니다')).toBeVisible();
  });
