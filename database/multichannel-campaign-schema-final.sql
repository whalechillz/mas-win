-- 🚀 멀티채널 캠페인 통합 스키마 (모든 타입 호환성 수정 버전)
...
CREATE TABLE IF NOT EXISTS funnel_channel_strategies (
    id SERIAL PRIMARY KEY,
    funnel_stage_id UUID NOT NULL REFERENCES marketing_funnel_stages(id),  -- UUID로 수정
    channel_id INTEGER NOT NULL REFERENCES marketing_channels(id),
...
-- 7. 퍼널 단계별 기본 채널 전략 데이터 삽입 (UUID 타입 대응)
...
-- 12. 함수: 캠페인 생성 시 자동 멀티채널 계획 생성 (간소화 버전)
