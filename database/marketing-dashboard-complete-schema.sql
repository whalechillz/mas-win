-- 🚀 마케팅 대시보드 전체 데이터베이스 스키마
-- campaigns 테이블이 없는 경우를 위한 전체 스키마

-- 1. campaigns 테이블 생성 (없는 경우)
...
-- 3. marketing_funnel_stages 테이블 생성 (없는 경우)
...
-- 기존 캠페인이 있다면 multichannel 필드 업데이트
UPDATE campaigns 
SET is_multichannel = true 
WHERE type = 'monthly_promotion' 
AND is_multichannel IS NULL;

-- 확인 쿼리
SELECT 'campaigns 테이블 생성 완료' as status, count(*) as count FROM campaigns
UNION ALL
SELECT 'marketing_channels 테이블 생성 완료', count(*) FROM marketing_channels
UNION ALL
SELECT 'monthly_themes 테이블 생성 완료', count(*) FROM monthly_themes
UNION ALL
SELECT 'marketing_funnel_stages 테이블 생성 완료', count(*) FROM marketing_funnel_stages;