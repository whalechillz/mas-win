...
async function testAIImprovement() {
  let browser;
  try {
    console.log('🚀 AI 개선 기능 테스트 시작...');
    ...
    if (await improvementButton.isVisible()) {
      ...
      improvementButton = page.locator('button:has-text("AI 개선")');
      if (await improvementButton.isVisible()) {
        console.log('✅ AI 개선 버튼 발견 (부분 텍스트)');
      } else {
...
이 내용이 AI에 의해 어떻게 개선되는지 확인해보겠습니다.
    ...
    await page.screenshot({ 
      path: 'ai-improvement-test-result.png',
      fullPage: true 
    });
    console.log('✅ 스크린샷 저장: ai-improvement-test-result.png');
    ...
    if (browser) {
      const page = await browser.newPage();
      await page.screenshot({ 
        path: 'ai-improvement-test-error.png',
        fullPage: true 
      });
      console.log('📸 에러 스크린샷 저장: ai-improvement-test-error.png');
    }
  } finally {
    console.log('🔚 10. 브라우저 종료...');
    await browser.close();
    console.log('✅ AI 개선 기능 테스트 완료');
  }
}

// 테스트 실행
testAIImprovement().catch(console.error);
