...
async function testAIButtonFixed() {
  console.log('🚀 Chrome Canary로 AI 버튼 테스트 (수정된 버전)...');
  ...
  try {
    ...
    // "새 게시물 작성" 버튼 클릭
    console.log('🖱️ "새 게시물 작성" 버튼 클릭...');
    const newPostButton = page.locator('button:has-text("새 게시물 작성")');
    ...
    if (hasTitleInput) {
      ...
      if (hasAIButton) {
        ...
        console.log(`✅ 성공 메시지: ${hasSuccess ? '있음' : '없음'}`);
...
// 스크립트 실행
testAIButtonFixed().catch(console.error);
