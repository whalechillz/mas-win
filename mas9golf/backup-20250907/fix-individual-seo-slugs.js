...
async function fixIndividualSEOSlugs() {
  ...
  try {
    ...
    const blogPosts = await page.evaluate(() => {
      const posts = [];
      const rows = document.querySelectorAll('tr[data-hook*="post"], .blog-post-row, tbody tr');
      
      rows.forEach((row, index) => {
        const titleElement = row.querySelector('td:nth-child(2), .post-title, [data-hook*="title"]');
        const urlElement = row.querySelector('td:nth-child(3), .post-url, [data-hook*="url"]');
        const editButton = row.querySelector('button:has-text("편집"), .edit-button, [data-hook*="edit"]');
    ...
    // 첫 5개 게시물만 처리 (테스트용)
    const postsToProcess = blogPosts.slice(0, 5);
    ...
    for (let i = 0; i < postsToProcess.length; i++) {
      ...
      try {
        // 게시물 편집 버튼 클릭
        const editButton = await page.locator(`tr:nth-child(${post.index + 1}) button:has-text("편집")`).or(
          page.locator(`tr:nth-child(${post.index + 1}) .edit-button`)
        );
