...
const BLOG_ALL_POSTS_DIR = path.join(BLOG_DIR, 'all-162-posts');
...
async function ensureDirs() {
  ...
  await fs.mkdir(BLOG_ALL_POSTS_DIR, { recursive: true });
}

async function getAllBlogPostUrls(page) {
  console.log('🔍 Wix 대시보드에서 모든 블로그 포스트 URL 수집...');
  ...
  try {
    ...
    while (hasMorePages && pageNumber <= 20) { // 최대 20페이지까지 (안전장치)
      console.log(`📄 페이지 ${pageNumber} 수집 중...`);
      ...
      // 다음 페이지로 이동 시도
      try {
        const nextButton = await page.$('button[aria-label*="다음"], button[aria-label*="Next"], [data-testid*="next"]');
        if (nextButton) {
    ...
    console.error('❌ URL 수집 중 오류:', error);
    return [];
  }
}
...
async function backupSinglePost(page, url, index, total) {
  try {
    ...
    await fs.writeFile(
      path.join(BLOG_ALL_POSTS_DIR, postFileName),
...
async function backupAll162BlogPosts() {
  console.log('📝 162개 블로그 글 완전 백업 시작!');
  await ensureDirs();
  ...
  try {
    ...
    // 1. 모든 블로그 포스트 URL 수집
    const blogUrls = await getAllBlogPostUrls(page);
    ...
    // 전체 블로그 데이터 저장
    await fs.writeFile(
      path.join(BLOG_DIR, 'all-162-blog-posts.json'),
    ...
    // 최종 요약 보고서 저장
    await fs.writeFile(
      path.join(BLOG_DIR, '162-blog-backup-summary.json'),
      JSON.stringify(blogSummary, null, 2),
      'utf8'
    );
    
    console.log(`🎉 162개 블로그 완전 백업 완료!`);
    console.log(`📊 최종 결과:`);
    ...
    console.log(`📄 개별 포스트: ${BLOG_ALL_POSTS_DIR}/`);
    console.log(`📄 전체 데이터: ${BLOG_DIR}/all-162-blog-posts.json`);
    console.log(`📄 최종 요약: ${BLOG_DIR}/162-blog-backup-summary.json`);
    
  } catch (error) {
    console.error('❌ 162개 블로그 백업 중 오류 발생:', error);
...
backupAll162BlogPosts().catch(console.error);
