...
async function downloadImage(page, imageUrl, imageIndex) {
  try {
    const response = await page.goto(imageUrl, { waitUntil: 'networkidle', timeout: 15000 });
  ...
  try {
    ...
    if (postData.images && postData.images.length > 0) {
      ...
      for (let i = 0; i < postData.images.length; i++) {
        ...
        const downloadedPath = await downloadImage(page, image.src, `${index}-${i}`);
        if (downloadedPath) {
...
async function backupBlog33PagesComplete() {
  console.log('📝 33페이지까지 모든 블로그 글 완전 백업 시작!');
  await ensureDirs();
  ...
  try {
    ...
    for (let i = 0; i < blogUrls.length; i++) {
      ...
      if ((i + 1) % 10 === 0) {
        ...
        await fs.writeFile(
          path.join(BLOG_DIR, 'progress-33-pages.json'),
    ...
    // 전체 블로그 데이터 저장
    await fs.writeFile(
      path.join(BLOG_DIR, 'all-blog-posts-33-pages.json'),
    ...
    // 최종 요약 보고서 저장
    await fs.writeFile(
      path.join(BLOG_DIR, 'blog-backup-33-pages-summary.json'),
      JSON.stringify(blogSummary, null, 2),
      'utf8'
    );
    
    console.log(`🎉 33페이지 블로그 백업 완료!`);
    console.log(`📊 최종 결과:`);
    ...
    console.log(`📄 전체 데이터: ${BLOG_DIR}/all-blog-posts-33-pages.json`);
    console.log(`📄 최종 요약: ${BLOG_DIR}/blog-backup-33-pages-summary.json`);
...
backupBlog33PagesComplete().catch(console.error);
