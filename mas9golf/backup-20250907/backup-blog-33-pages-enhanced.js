...
async function loadExistingData() {
  try {
    // 기존 백업 데이터 로드
    const existingFile = path.join(BLOG_DIR, 'all-blog-posts-33-pages.json');
    const existingData = await fs.readFile(existingFile, 'utf8');
    const existingPosts = JSON.parse(existingData);
    
    // 데이터 유효성 검사
    if (Array.isArray(existingPosts) && existingPosts.length > 0) {
      console.log(`📂 기존 백업 데이터 로드: ${existingPosts.length}개 포스트`);
      return existingPosts;
    } else {
      console.log('📂 기존 백업 데이터가 비어있음, 새로 시작합니다.');
      return [];
    }
  } catch (error) {
    console.log('📂 기존 백업 데이터 없음, 새로 시작합니다.');
    return [];
  }
}
...
async function downloadHighResImage(page, imageUrl, imageIndex, existingImage = null) {
  try {
    ...
    // 기존 이미지와 비교
    if (existingImage && existingImage.src === highResUrl) {
      return existingImage.localPath;
    }
...
async function backupSinglePost(page, url, index, total, existingPost = null, isRetry = false) {
  try {
    ...
    if (postData.images && postData.images.length > 0) {
      ...
      for (let i = 0; i < postData.images.length; i++) {
        ...
        const existingImage = existingPost?.images?.[i];
        const downloadedPath = await downloadHighResImage(page, image.src, `${index}-${i}`, existingImage);
        if (downloadedPath) {
    ...
    // 기존 포스트가 있으면 업데이트, 없으면 새로 추가
    if (existingPost) {
      const existingIndex = blogSummary.posts.findIndex(p => p.url === url);
      if (existingIndex >= 0) {
        blogSummary.posts[existingIndex] = postData;
      }
    } else {
...
async function backupBlog33PagesEnhanced() {
  ...
  try {
    ...
    // 1. 기존 백업 데이터 로드
    const existingPosts = await loadExistingData();
    ...
    // 3. 중복 제거 및 새 포스트 식별
    const newUrls = blogUrls.filter(url => !existingUrls.has(url));
    const existingUrlsToUpdate = blogUrls.filter(url => existingUrls.has(url));
    ...
    if (existingPosts && Array.isArray(existingPosts)) {
      existingPosts.forEach(post => {
        if (post && post.url) {
          existingPostMap.set(post.url, post);
        }
      });
    }
    
    console.log(`📊 URL 분석:`);
    console.log(`   - 총 발견된 URL: ${blogUrls.length}개`);
    console.log(`   - 기존 백업된 URL: ${existingUrlsToUpdate.length}개`);
    console.log(`   - 새로운 URL: ${newUrls.length}개`);
    
    blogSummary.totalPosts = blogUrls.length;
    blogSummary.progress.total = blogUrls.length;
    
    // 4. 기존 포스트 업데이트 (고해상도 이미지)
    console.log(`🔄 기존 포스트 업데이트 시작 (고해상도 이미지)...`);
    for (let i = 0; i < existingUrlsToUpdate.length; i++) {
      const url = existingUrlsToUpdate[i];
      const existingPost = existingPostMap.get(url);
      await backupSinglePost(page, url, i + 1, blogUrls.length, existingPost, false);
      
      if ((i + 1) % 10 === 0) {
        console.log(`💾 기존 포스트 업데이트 진행: ${i + 1}/${existingUrlsToUpdate.length} 완료`);
      }
    ...
    // 5. 새로운 포스트 백업
    console.log(`🆕 새로운 포스트 백업 시작...`);
    for (let i = 0; i < newUrls.length; i++) {
      const url = newUrls[i];
      await backupSinglePost(page, url, existingUrlsToUpdate.length + i + 1, blogUrls.length, null, false);
      
      if ((i + 1) % 10 === 0) {
        console.log(`💾 새 포스트 백업 진행: ${i + 1}/${newUrls.length} 완료`);
      }
    ...
    if (blogSummary.retryList.length > 0) {
      ...
      for (let i = 0; i < blogSummary.retryList.length; i++) {
        ...
        try {
          await backupSinglePost(page, retryItem.url, retryItem.index, blogUrls.length, null, true);
    ...
    // 전체 블로그 데이터 저장
    await fs.writeFile(
      path.join(BLOG_DIR, 'all-blog-posts-33-pages-enhanced.json'),
    ...
    // 최종 요약 보고서 저장
    await fs.writeFile(
      path.join(BLOG_DIR, 'blog-backup-33-pages-enhanced-summary.json'),
    ...
    console.log(`📄 전체 데이터: ${BLOG_DIR}/all-blog-posts-33-pages-enhanced.json`);
    console.log(`📄 최종 요약: ${BLOG_DIR}/blog-backup-33-pages-enhanced-summary.json`);
...
backupBlog33PagesEnhanced().catch(console.error);
