...
async function setupGoogleVisionAPI() {
  ...
  try {
    ...
    await page.waitForURL('**/console.cloud.google.com/**', { timeout: 60000 });
    
    // 2단계: 프로젝트 선택 또는 생성
    console.log('📝 2단계: 프로젝트 선택/생성 중...');
    
    // 프로젝트 선택기 클릭
    const projectSelector = await page.waitForSelector('[data-testid="project-selector"]', { timeout: 10000 });
    if (projectSelector) {
      await projectSelector.click();
      console.log('✅ 프로젝트 선택기 클릭됨');
    }
    
    // 새 프로젝트 생성 버튼 클릭 (선택사항)
    const newProjectBtn = await page.waitForSelector('text="새 프로젝트"', { timeout: 5000 });
    if (newProjectBtn) {
      await newProjectBtn.click();
      console.log('✅ 새 프로젝트 생성 버튼 클릭됨');
      
      // 프로젝트 이름 입력
      const projectName = `masgolf-vision-api-${Date.now()}`;
    ...
    console.log('📝 3단계: Cloud Vision API 활성화 중...');
    
    // API 및 서비스 메뉴로 이동
    await page.goto('https://console.cloud.google.com/apis/library/vision.googleapis.com');
    ...
    // 5단계: API 키 제한 설정
    console.log('📝 5단계: API 키 제한 설정 중...');
    
    // API 키 제한 버튼 클릭
    const restrictBtn = await page.click('text="제한사항"');
    
    // 애플리케이션 제한: HTTP 리퍼러 선택
    await page.click('input[value="HTTP_REFERRERS"]');
    
    // 리퍼러 추가
    await page.fill('textarea[placeholder="리퍼러 추가"]', 'http://localhost:3000/*');
    await page.fill('textarea[placeholder="리퍼러 추가"]', 'https://win.masgolf.co.kr/*');
    
    // API 제한: Cloud Vision API 선택
    await page.click('input[value="API_RESTRICTIONS"]');
    await page.selectOption('select', 'Cloud Vision API');
    
    // 저장 버튼 클릭
    await page.click('button:has-text("저장")');
    console.log('✅ API 키 제한 설정 완료');
    
    // 6단계: .env.local 파일 업데이트
    console.log('📝 6단계: .env.local 파일 업데이트 중...');
    ...
    console.log('📋 설정된 내용:');
    console.log(`🔑 API 키: ${apiKey}`);
    console.log('🔒 제한사항: HTTP 리퍼러, Cloud Vision API만');
