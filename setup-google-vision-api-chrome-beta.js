...
async function setupGoogleVisionAPI() {
  console.log('🚀 Google Vision API 키 발급 자동화 시작 (Chrome Beta)');
  console.log('=====================================================');
...
function showImportantNotes() {
  ...
  console.log('');
  console.log('⚠️ 주의사항:');
  console.log('  - Chrome Beta 브라우저가 열리면 Google 계정으로 로그인해주세요');
  console.log('  - 로그인 완료 후 터미널에서 Enter 키를 눌러주세요');
  console.log('  - API 키 생성 후 자동으로 .env.local 파일이 업데이트됩니다');
  console.log('  - Chrome Beta가 설치되어 있지 않으면 일반 Chrome으로 실행됩니다');
}
