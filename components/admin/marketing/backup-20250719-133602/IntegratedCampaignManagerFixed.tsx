...
// 통합 마케팅 캠페인 매니저 - 월별 테마 관리 기능 포함 (유료화 제거)
export const IntegratedCampaignManager = ({ supabase }) => {
  ...
  const generateMultiChannelContent = async () => {
    ...
    const confirmMessage = aiSettings.useAI 
      ? `${selectedYear}년 ${selectedMonth}월의 모든 멀티채널 콘텐츠를 ${aiSettings.model}로 자동 생성하시겠습니까?`
      ...
      <div className="mb-6 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-4">
        ...
        {aiSettings.useAI && (
          <div className="mt-2 flex items-center gap-2 text-xs text-gray-600">
            <span>현재 AI 모델:</span>
            <span className="font-medium text-purple-700">{aiSettings.model}</span>
            {aiSettings.settings.usePerplexity && <span className="text-blue-600">+ Perplexity</span>}
            {aiSettings.settings.useImageGen && <span className="text-green-600">+ 이미지 생성</span>}
          </div>
        )}
      </div>
