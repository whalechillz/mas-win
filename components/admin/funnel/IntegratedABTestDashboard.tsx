...
export function IntegratedABTestDashboard() {
  ...
  useEffect(() => {
    ...
    // 5분마다 자동 업데이트
    const interval = setInterval(fetchABTestData, 5 * 60 * 1000);
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">실시간 A/B 테스트 데이터를 로드하는 중...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">실시간 A/B 테스트 성능 비교</h2>
          <p className="text-gray-600">
            {formatDate(testDateRange.startDate)} ~ {formatDate(testDateRange.endDate)}
          </p>
          <p className="text-sm text-gray-500">
            총 {totalVersions}개 버전 테스트 중 • 마지막 업데이트: {lastUpdate}
          </p>
        </div>
...
export default IntegratedABTestDashboard;
