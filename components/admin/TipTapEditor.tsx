...
export const TipTapEditor: React.FC<TipTapEditorProps> = ({ valueMarkdown, onChangeMarkdown, onRequestImageFromGallery }) => {
  ...
  const editorOptions = useMemo(() => {
    try {
      return {
        ...
        editorProps: { attributes: { class: 'prose max-w-none min-h-[240px] focus:outline-none' } },
        onUpdate: ({ editor }: any) => {
          try {
            // @ts-ignore - storage provided by Markdown extension
            const md = editor.storage?.markdown?.getMarkdown?.();
            onChangeMarkdown((md as string) ?? editor.getText());
          } catch {
  ...
  }, [valueMarkdown, onChangeMarkdown]);
  ...
  // Fallback textarea when TipTap initialization fails
  if (fallback || !editor) {
    return (
