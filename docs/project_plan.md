...
#### 모바일 모달에서 이미지가 안 나오는 문제 해결 ✅
- **문제**: 모바일 사파리와 크롬에서 "갤러리에서 선택" 모달 내부 이미지가 표시되지 않음 (본창에서는 정상 작동)
- **원인**: 
  - 모달의 `overflow-hidden`과 `fixed` position 조합이 모바일 브라우저에서 이미지 렌더링을 방해
  - 이미지에 `loading` 속성이 없어 lazy loading이 모달 내부에서 제대로 작동하지 않음
  - 모바일에서 `backdrop-blur`가 성능 문제를 일으켜 이미지 렌더링 지연
- **해결**:
  - 모달 내부 컨테이너의 `overflow-hidden` 제거 (1057번 라인)
  - 이미지에 `loading="eager"` 및 `decoding="async"` 추가 (1731번, 996번 라인)
  - 이미지 로딩 에러 처리 추가 (`onError` 핸들러)
  - 모바일에서 `backdrop-blur` 제거하여 성능 개선 (1056번 라인)
- **수정 파일**:
  - `components/admin/GalleryPicker.tsx`: 모달 구조 및 이미지 로딩 최적화

## ✅ 이전 작업: 갤러리 선택 모달 에러 처리 및 모바일 UX 개선 (2026-01-24)

### 완료된 작업

#### 아이폰 이미지 로딩 에러 처리 개선 ✅
- **문제**: 아이폰에서 이미지가 로드되지 않는 문제 (504 Gateway Timeout, JSON 파싱 에러)
- **원인**: 
  - 504 타임아웃 발생 시 사용자에게 명확한 안내 부족
  - JSON 파싱 에러 발생 시 (HTML 응답) 처리 부족
  - 에러 발생 시 재시도 옵션 부재
- **해결**:
  - `loadError` 상태 추가하여 에러 메시지 관리
  - 504 타임아웃 시 모바일/PC별 맞춤 에러 메시지 표시
  - JSON 파싱 에러 처리 추가 (HTML 응답 대응)
  - 에러 UI 추가: 이미지 그리드 상단에 에러 메시지 및 "다시 시도" 버튼 표시
- **수정 파일**:
  - `components/admin/GalleryPicker.tsx`: 
    - 75번 라인: `loadError` 상태 추가
    - 233-250번 라인: 에러 처리 로직 개선 (에러 메시지 설정, JSON 파싱 에러 처리)
    - 1574번 라인 근처: 에러 UI 추가 (에러 메시지 및 재시도 버튼)

#### 이미지 없을 때 설명 간소화 ✅
- **문제**: 이미지가 없을 때 모바일에서도 키보드 단축키 안내가 표시되어 복잡함
- **원인**: 
  - "Shift + 드롭 = 링크 | Ctrl/Cmd + 드롭 = 복사" 안내가 모바일에서 의미 없음
  - 모바일에서는 드래그 앤 드롭이 제한적
- **해결**:
  - 모바일에서는 키보드 단축키 안내 제거
  - 모바일 친화적인 안내로 변경: "이미지를 업로드하려면 위의 업로드 버튼을 사용하세요."
  - PC는 기존 드래그 앤 드롭 안내 유지
- **수정 파일**:
  - `components/admin/GalleryPicker.tsx`: 1566-1569번 라인 - 모바일/PC 조건부 안내 메시지

#### 모바일 클린업 기능 개선 ✅
- **문제**: 모바일에서 클린업 기능 사용 시 "드래그 앤 드롭" 안내가 부적절함
- **원인**: 
  - cleanup.pictures는 모바일에서도 접근 가능하지만, 파일 업로드 방식이 다름
  - 모바일에서는 드래그 앤 드롭 대신 파일 선택(input) 방식 사용
- **해결**:
  - 모바일에서는 "파일 선택" 방식 안내로 변경
  - PC는 기존 "드래그 앤 드롭" 안내 유지
  - 모바일 안내: "Upload Image" 버튼 클릭 → 이미지 선택 → 편집 → 다운로드
- **수정 파일**:
  - `components/admin/GalleryPicker.tsx`: 
    - 1832-1843번 라인 (컴팩트 모드): 모바일/PC 조건부 클린업 안내 메시지
    - 1956-1968번 라인 (비-컴팩트 모드): 모바일/PC 조건부 클린업 안내 메시지

#### 모바일 다운로드 후 업로드 UX 개선 ✅
- **문제**: 모바일에서 이미지 다운로드 후 업로드 워크플로우가 불편함
- **원인**: 
  - 다운로드 완료 후 사용자가 수동으로 업로드 버튼을 찾아야 함
  - 다운로드 성공 피드백이 부족함
- **해결**:
  - 모바일에서 다운로드 완료 시 업로드 안내 추가
  - confirm 다이얼로그로 업로드 여부 확인
  - 업로드 선택 시 파일 input 자동 클릭하여 업로드 모달 열기
  - PC는 기존 간단한 알림 유지
- **수정 파일**:
  - `components/admin/GalleryPicker.tsx`: 
    - 1808-1828번 라인 (컴팩트 모드): 모바일 다운로드 후 업로드 안내 추가
    - 1912-1932번 라인 (비-컴팩트 모드): 모바일 다운로드 후 업로드 안내 추가
