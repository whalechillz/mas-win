...
## ✅ 최근 작업: 갤러리 선택 모달 에러 처리 및 모바일 UX 개선 (2026-01-24)

### 완료된 작업

#### 아이폰 이미지 로딩 에러 처리 개선 ✅
- **문제**: 
  1. 아이폰에서 이미지가 로드되지 않음 (504 Gateway Timeout 에러)
  2. JSON 파싱 에러 발생 (HTML 에러 페이지를 JSON으로 파싱)
  3. 에러 발생 시 사용자에게 명확한 안내 부족
- **원인**: 
  1. 서버 응답 지연으로 인한 504 타임아웃
  2. 에러 응답이 HTML 형식으로 반환되어 JSON 파싱 실패
  3. 에러 상태를 UI에 표시하지 않아 사용자가 원인 파악 어려움
- **해결**:
  1. **에러 상태 관리 추가**: `loadError` 상태 변수 추가
  2. **에러 메시지 개선**: 
     - 504 타임아웃: 모바일/PC에 맞는 안내 메시지
     - 500+ 서버 오류: 일반적인 서버 오류 안내
  3. **JSON 파싱 에러 처리**: try-catch로 JSON 파싱 실패 시 명확한 에러 메시지
  4. **에러 UI 추가**: 이미지 그리드 상단에 에러 메시지 및 재시도 버튼 표시
  5. **재시도 기능**: 사용자가 직접 재시도할 수 있는 버튼 제공
- **수정 파일**:
  - `components/admin/GalleryPicker.tsx`: 에러 상태 추가, 에러 처리 개선, 에러 UI 추가 (69, 233-250, 1574번 라인)

#### 이미지 없을 때 설명 간소화 ✅
- **문제**: 이미지가 없을 때 표시되는 설명이 모바일에서 복잡함
- **원인**: 
  - "Shift + 드롭 = 링크 | Ctrl/Cmd + 드롭 = 복사" 같은 키보드 단축키 안내가 모바일에서 의미 없음
  - 모바일에서는 드래그 앤 드롭이 제한적
- **해결**:
  - 모바일에서는 키보드 단축키 안내 제거
  - 모바일 친화적인 안내로 변경: "이미지를 업로드하려면 위의 업로드 버튼을 사용하세요."
  - PC에서는 기존 드래그 앤 드롭 안내 유지
- **수정 파일**:
  - `components/admin/GalleryPicker.tsx`: 이미지 없음 설명 모바일 최적화 (1566-1569번 라인)

#### 모바일 클린업 기능 개선 ✅
- **문제**: 모바일에서 클린업 안내 메시지가 "드래그 앤 드롭" 위주로 되어 모바일 UX에 부적절
- **원인**: 
  - cleanup.pictures는 모바일에서도 접근 가능하지만, 안내 메시지가 PC 중심
  - 모바일에서는 파일 선택(input) 방식으로 이미지 업로드 가능
- **해결**:
  - 모바일: "Upload Image" 버튼 클릭 → 파일 선택 안내
  - PC: 기존 "드래그 앤 드롭" 안내 유지
  - 컴팩트 모드와 비-컴팩트 모드 모두에 적용
- **수정 파일**:
  - `components/admin/GalleryPicker.tsx`: 클린업 안내 메시지 모바일 최적화 (1832-1844, 1936-1955번 라인)

#### 모바일 다운로드 후 업로드 UX 개선 ✅
- **문제**: 모바일에서 이미지 다운로드 후 업로드하는 워크플로우가 불편함
- **원인**: 
  - 다운로드 완료 후 사용자가 수동으로 업로드 버튼을 찾아야 함
  - 다운로드와 업로드가 분리되어 있어 워크플로우가 끊김
- **해결**:
  1. 모바일에서 다운로드 완료 시 업로드 안내 추가
  2. confirm 다이얼로그로 업로드 여부 확인
  3. 업로드 선택 시 파일 input 자동 클릭하여 업로드 모달 열기
  4. PC에서는 간단한 알림만 표시 (기존 동작 유지)
  5. 컴팩트 모드와 비-컴팩트 모드 모두에 적용
- **수정 파일**:
  - `components/admin/GalleryPicker.tsx`: 다운로드 후 업로드 안내 추가 (1857-1887, 1968-2003번 라인)
