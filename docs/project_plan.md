...
## ✅ 최근 작업: 갤러리 선택 모달 UI 개선 (2026-01-24)
...
- **문제**: 
  ...
  5. 아이폰에서 이미지가 표시되지 않음
  6. 이미지가 없을 때 설명이 너무 김
- **해결**:
  ...
  7. **아이폰 이미지 표시 문제 해결**:
     - Tailwind 동적 클래스 문제 해결: IIFE를 사용하여 명시적 클래스명 반환
     - `grid-cols-${variable}` → 조건문으로 전체 클래스명 반환
  8. **이미지 없음 설명 간소화**:
     - 긴 설명 텍스트 제거
     - 폴더 경로 표시 제거
     - 팁 메시지 제거
     - 모바일에서 드래그 앤 드롭 안내 제거
     - "이미지가 없습니다" + "상위 폴더 보기" 버튼만 표시
- **수정 파일**:
  - `components/admin/GalleryPicker.tsx`: 썸네일 버튼 표시 개선, 비교 모드 설명 삭제, 컬럼 수 변경 기능 추가, 버튼 레이아웃 개선, 태블릿 감지 추가, 아이폰 이미지 표시 수정, 이미지 없음 설명 간소화
