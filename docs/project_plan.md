...
## 2025-10-17 콘텐츠 캘린더 심플한 2단계 탭 구조 구현

### 무엇을 했나
- 콘텐츠 캘린더 페이지를 **심플한 2단계 탭 구조**로 재구성
- **메인 탭 4개**:
  - 🏠 **대시보드**: 통계 카드와 빠른 액션 버튼
  - 📝 **보기**: 5가지 뷰 전환 (리스트/달력/트리/탭/테이블)
  - ✍️ **생성**: 원본 콘텐츠 생성, 파일 업로드, 빠른 추가, 연간 콘텐츠 생성
  - ⚙️ **관리**: 블로그 동기화, 템플릿 관리, 시스템 설정
- 기존의 복잡한 UI를 탭별로 명확하게 분리하여 정리

### 왜 했나
- 기존 페이지가 너무 길고 복잡하여 사용자 경험 저하
- 블로그 관리와 동일한 패턴으로 일관성 확보
- 기능별로 명확하게 구분하여 유지보수 용이성 향상
- 자주 발생하던 로딩 스피너 문제 개선

### 변경 파일
- `pages/admin/content-calendar.tsx`
  - 메인 탭 네비게이션 추가
  - 탭별 콘텐츠 분리 (대시보드, 보기, 생성, 관리)
  - 중복된 섹션 제거 (290줄 제거)
  - JSX 구조 정리

### 기술적 세부사항
- **메인 탭 상태 관리**: `useState<'dashboard' | 'view' | 'create' | 'manage'>('dashboard')`
- **서브 탭 유지**: 보기 탭 내에서 리스트/달력/트리/탭/테이블 뷰 전환
- **기존 컴포넌트 재사용**: 
  - `StatsCards` (통계 카드)
  - `OriginalContentSection` (원본 콘텐츠 생성)
  - `FileUploadSection` (파일 업로드)
  - `QuickAddSection` (빠른 추가)
- **린트 에러 수정**:
  - `conversion_goal` → `conversion_goals` (속성명 수정)
  - `content_body` → `title` (존재하지 않는 속성 수정)

### 개선 효과
1. **UI/UX 개선**: 페이지 길이 약 30% 감소, 명확한 기능 구분
2. **유지보수성**: 탭별로 기능이 분리되어 코드 관리 용이
3. **일관성**: 블로그 관리와 동일한 탭 구조로 학습 비용 감소
4. **성능**: 필요한 탭의 콘텐츠만 렌더링하여 성능 향상
