...
// SEO 점수 계산 함수
function calculateSEOScore(content: string, metadata: any, keywords: string[]): number {
  let score = 0;
  ...
  try {
    switch (method) {
      case 'POST':
        ...
        // 키워드 결정 (전달받은 키워드 또는 메타데이터의 키워드 사용)
        ...
        const { data: updatedContent, error: updateError } = await supabase
          ...
          .update({
            ...
            status: seoScore >= 70 && readabilityScore >= 70 ? 'validated' : 'draft'
          })
          .eq('id', content_id)
          .select()
          .single();
      ...
      case 'GET':
        // 검증된 콘텐츠 조회
        const { funnel_plan_id, status = 'validated' } = query;
        ...
        if (funnel_plan_id) getQuery = getQuery.eq('funnel_plan_id', funnel_plan_id);
        if (status) getQuery = getQuery.eq('status', status);

        const { data, error } = await getQuery.order('updated_at', { ascending: false });
