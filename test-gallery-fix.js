...
async function testGalleryFix() {
  ...
  try {
        ...
        if (await featuredImageSection.isVisible()) {
          ...
          if (await featuredImageInput.isVisible()) {
            ...
            if (galleryButton) {
              ...
              // 7. Supabase 저장 버튼 테스트
              console.log('💾 7. Supabase 저장 버튼 테스트...');
              
              // 대표 이미지 관리 섹션으로 다시 스크롤
              await featuredImageSection.scrollIntoViewIfNeeded();
              await page.waitForTimeout(1000);
              ...
              if (await saveButton.isVisible()) {
                ...
                if (newValue !== currentValue) {
                  ...
                  const newImageCount = await galleryImages.count();
                  console.log(`📊 저장 후 갤러리 이미지 개수: ${newImageCount}개`);
                  
                  if (newImageCount > 0) {
                    console.log('✅ 갤러리에 이미지가 추가되었습니다!');
                    
                    // 새로 추가된 이미지 확인
                    for (let i = 0; i < Math.min(newImageCount, 3); i++) {
                      const img = galleryImages.nth(i);
                      const src = await img.getAttribute('src');
                      console.log(`  ${i + 1}. ${src}`);
    ...
    await page.screenshot({ 
      path: 'test-gallery-fix-result.png',
      fullPage: true 
    });
    console.log('✅ 스크린샷 저장: test-gallery-fix-result.png');
    ...
    if (browser) {
      const page = await browser.newPage();
      await page.screenshot({ 
        path: 'test-gallery-fix-error.png',
        fullPage: true 
      });
      console.log('📸 에러 스크린샷 저장: test-gallery-fix-error.png');
    }
  } finally {
    console.log('🔚 10. 브라우저 종료...');
    await browser.close();
    console.log('✅ 갤러리 수정 사항 테스트 완료');
  }
}

// 테스트 실행
testGalleryFix().catch(console.error);
