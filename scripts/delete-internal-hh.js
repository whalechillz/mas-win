/**
 * 내부자 "hh" 예약 및 고객 삭제 스크립트
 * 
 * 이름에 "hh"가 포함된 모든 예약과 고객을 삭제합니다.
 * 
 * 사용법:
 * node scripts/delete-internal-hh.js [--dry-run]
 * 
 * 옵션:
...
async function deleteInternalHH() {
  const dryRun = process.argv.includes('--dry-run');
  
  console.log('🔍 "hh" 포함 예약 및 고객 조회 중...\n');
  if (dryRun) {
    console.log('⚠️  DRY RUN 모드: 실제 삭제 없이 확인만 수행합니다.\n');
  }
  
  // 예약 조회 (이름에 hh 포함, 대소문자 구분 없음)
  ...
  if (bookingCount === 0 && customerCount === 0) {
    console.log('✅ "hh"가 포함된 예약이나 고객이 없습니다.\n');
    return;
  }
