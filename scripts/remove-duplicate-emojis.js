...
// 중복 이모지 제거 스크립트
async function removeDuplicateEmojis() {
  try {
    console.log('🔧 중복 이모지 제거 시작...');
    ...
    console.log(`📝 이모지 제거된 콘텐츠 길이: ${postData.content.length}자`);
    
    // 파일 저장
    await fs.writeFile(postFilePath, JSON.stringify(postData, null, 2), 'utf8');
    
    console.log('✅ 중복 이모지 제거 완료');
...
export default function handler(req, res) {
  ...
  res.status(200).json({
    ...
    console.log('\n🎉 중복 이모지 제거 완료!');
    console.log('📊 해결 결과:');
    console.log(`  🗑️ "🎯 주요 특징" → "주요 특징"`);
    console.log(`  🗑️ "📞 문의 및 예약" → "문의 및 예약"`);
    console.log(`  ✅ 아이콘만으로 깔끔한 디자인`);
    console.log(`  🎨 7월 퍼널 스타일 유지`);
    
  } catch (error) {
    console.error('❌ 중복 이모지 제거 중 오류 발생:', error);
  }
}
...
module.exports = { removeDuplicateEmojis };
