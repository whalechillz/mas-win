/**
 * 80번 메시지의 이미지를 로컬 파일에서 복구하는 스크립트
 * 
 * 사용법:
 * 1. 솔라피 콘솔에서 이미지를 "다른 이름으로 저장"으로 다운로드
 * 2. node scripts/recover-message-80-image-from-file.js <이미지파일경로>
 * 
 * 예시:
 * node scripts/recover-message-80-image-from-file.js ~/Downloads/solapi-image.jpg
 */
...
async function recoverMessage80ImageFromFile(imageFilePath) {
  console.log('\n🔄 80번 메시지 이미지 복구 시작 (로컬 파일에서)...\n');
  ...
  try {
    ...
    if (!imageFilePath) {
      ...
      console.log('\n사용법:');
      console.log('  node scripts/recover-message-80-image-from-file.js <이미지파일경로>');
      console.log('\n예시:');
      console.log('  node scripts/recover-message-80-image-from-file.js ~/Downloads/solapi-image.jpg');
      process.exit(1);
    }
...
recoverMessage80ImageFromFile(imageFilePath)
  .then(() => {
    console.log('✅ 완료!');
    process.exit(0);
  })
