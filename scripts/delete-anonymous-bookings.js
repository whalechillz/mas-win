 ...
 * 무기명 예약 삭제 스크립트
 * 
 * 010-6669-9000 전화번호로 된 모든 예약과 고객을 삭제합니다.
 * 
 * 사용법:
 * node scripts/delete-anonymous-bookings.js [--dry-run]
 * 
 * 옵션:
...
async function deleteAnonymousBookings() {
  const dryRun = process.argv.includes('--dry-run');
  const phone = '01066699000'; // 하이픈 제거
  
  console.log('🔍 무기명 예약 및 고객 조회 중...\n');
  if (dryRun) {
    console.log('⚠️  DRY RUN 모드: 실제 삭제 없이 확인만 수행합니다.\n');
  }
  
  // 모든 형태의 전화번호로 조회
  const phoneConditions = `phone.eq.${phone},phone.eq.010-6669-9000`;
  ...
  console.log(`전화번호: 010-6669-9000 (무기명)`);
  console.log('');
