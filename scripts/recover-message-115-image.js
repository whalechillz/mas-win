...
async function recoverMessage115Image() {
  console.log('🔄 115번 메시지 이미지 복구 시작...\n');

  const messageId = 115;
  const groupId = 'G4V202511210005143EWKQVHZSSW2KHC';
  // 발송일은 메시지의 sent_at에서 추출
  ...
  try {
    ...
    console.log(`   솔라피 그룹 ID: ${message.solapi_group_id || groupId}`);
    console.log(`   발송일: ${message.sent_at || '없음'}\n`);

    // 발송일에서 날짜 추출 (YYYY-MM-DD)
    let sentDate = '2025-11-21'; // 기본값
    if (message.sent_at) {
    ...
    // 3. 이미지 ID 추출 (다양한 필드명 시도)
    let imageId = null;
    
    if (messageListData.messages && messageListData.messages.length > 0) {
      const firstMessage = messageListData.messages[0];
      
      // RAW DATA에서 imageId 추출 (다양한 필드명 시도)
      imageId = firstMessage.imageId || firstMessage.image_id || firstMessage.imageId || null;
    ...
    if (!imageId) {
      ...
      console.log('   사용법: node scripts/recover-message-115-image.js <imageId>');
      process.exit(1);
    }
    ...
    // 방법 2: 다운로드 실패 시 사용자에게 안내
    if (!downloadSuccess) {
      console.error('❌ 솔라피 Storage API로 이미지 다운로드 실패');
      console.log('\n💡 대안 방법:');
      console.log('   1. 솔라피 콘솔에서 이미지를 직접 다운로드');
      console.log('   2. SMS 편집 페이지에서 이미지를 재업로드');
      console.log('   3. 또는 수동으로 imageId를 Supabase에 저장 (임시)');
      console.log(`\n   현재 imageId: ${imageId}`);
      console.log(`   channel_sms.image_url에 ${imageId}를 저장합니다 (임시)...\n`);

      // 임시로 Solapi imageId를 저장
      ...
      console.log('✅ channel_sms.image_url 업데이트 완료 (임시)');
      console.log('💡 나중에 Supabase에 이미지를 저장한 후 URL로 교체하세요.\n');
      process.exit(0);
    }

    // 6. Supabase Storage에 저장
    const dateFolder = sentDate; // 2025-11-21
    ...
    if (uploadError) {
      ...
      // 폴더가 없을 수 있으므로 폴더 생성 시도는 하지 않음 (자동 생성됨)
      // upsert로 재시도
    ...
    console.log('💡 SMS 편집 페이지에서 확인하세요: /admin/sms?id=115\n');
