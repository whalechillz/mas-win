 ...
 * 내부자 이은정 예약 삭제 스크립트
 * 
 * 이은정 (010-3243-3099) 전화번호로 된 모든 예약과 고객을 삭제합니다.
 * 
 * 사용법:
 * node scripts/delete-internal-lee-eun-jung.js [--dry-run]
 * 
 * 옵션:
...
async function deleteInternalLeeEunJung() {
  const dryRun = process.argv.includes('--dry-run');
  const phone = '01032433099'; // 하이픈 제거
  const name = '이은정';
  
  console.log('🔍 내부자 이은정 예약 및 고객 조회 중...\n');
  if (dryRun) {
    console.log('⚠️  DRY RUN 모드: 실제 삭제 없이 확인만 수행합니다.\n');
  }
  
  // 모든 형태의 전화번호로 조회
  const phoneConditions = `phone.eq.${phone},phone.eq.010-3243-3099`;
  ...
  console.log(`이름: ${name}`);
  console.log(`전화번호: 010-3243-3099 (내부자)`);
  console.log('');
  console.log('위 예약과 고객을 삭제하시겠습니까?');
  console.log('(자동 실행 중...)');
  console.log('');
