...
async function fixDuplicateTitles() {
  try {
    ...
    // 콘텐츠에서 h1 태그 제거 (페이지에서 이미 제목을 표시하므로)
    const cleanContent = `
    ...
    console.log('✅ 게시물 콘텐츠에서 제목 제거 완료');
...
export default function handler(req, res) {
  ...
  res.status(200).json({
    ...
    console.log('\n🎉 중복 제목 문제 해결 완료!');
    console.log('📊 해결 결과:');
    console.log(`  🗑️ 콘텐츠에서 h1 제목 제거`);
    console.log(`  📄 페이지 헤더에서만 제목 표시`);
    console.log(`  🎨 깔끔한 단일 제목 레이아웃`);
    
  } catch (error) {
    console.error('❌ 중복 제목 해결 중 오류 발생:', error);
  }
}
...
module.exports = { fixDuplicateTitles };
