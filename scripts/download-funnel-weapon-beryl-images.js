...
async function extractAndDownloadImages(browser, url, pagePath) {
  ...
  try {
    ...
    const htmlPath = path.join(process.cwd(), 'scraped-pages', `funnel-weapon-beryl.html`);
...
async function main() {
  ...
  const pagePath = '/funnel/weapon-beryl';
  const publicDir = path.join(process.cwd(), 'public');
  
  console.log('🔍 원본 사이트에서 weapon-beryl 페이지 이미지 추출 및 다운로드 시작...');
  ...
  try {
    ...
    // 다운로드된 이미지 목록 저장
    const downloadedListPath = path.join(process.cwd(), 'downloaded-weapon-beryl-images.json');
