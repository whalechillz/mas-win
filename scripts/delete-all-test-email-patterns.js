...
// 삭제할 테스트 이메일 패턴 목록 (대소문자 구분 없이 매칭)
...
async function deleteAllTestEmailPatterns() {
  const dryRun = process.argv.includes('--dry-run');
  
  console.log('🔍 모든 테스트 이메일 패턴 찾기 및 삭제 중...\n');
  if (dryRun) {
  ...
  for (const email of testEmailPatterns) {
    ...
    if (bookings && bookings.length > 0) {
      ...
      if (bookings.length > 5) {
        console.log(`   ... 외 ${bookings.length - 5}건`);
      }
  ...
  for (const email of testEmailPatterns) {
    ...
    if (customers && customers.length > 0) {
      ...
      if (customers.length > 5) {
        console.log(`   ... 외 ${customers.length - 5}건`);
      }
