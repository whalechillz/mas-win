/**
 * 263번 메시지의 이미지를 로컬 파일에서 복구하는 스크립트
 *
 * 사용법:
 * 1. Solapi 콘솔에서 해당 MMS 이미지를 "다른 이름으로 저장"으로 다운로드
 * 2. 아래 예시처럼 node 명령으로 실행
 *
 * 예시:
 *  node scripts/recover-message-263-image-from-file.js ~/Downloads/ST01FZ2512510202939395w6sR1vmZC52.jpeg
 */
...
async function recoverMessage263ImageFromFile(imageFilePath) {
  console.log('\n🔄 263번 메시지 이미지 복구 시작 (로컬 파일에서)...\n');
  ...
  try {
    // 1. 파일 경로 확인
    if (!imageFilePath) {
      console.error('❌ 이미지 파일 경로가 필요합니다.');
      console.log('\n사용법:');
      console.log('  node scripts/recover-message-263-image-from-file.js <이미지파일경로>');
      console.log('\n예시:');
      console.log('  node scripts/recover-message-263-image-from-file.js ~/Downloads/ST01FZ2512510202939395w6sR1vmZC52.jpeg');
      process.exit(1);
    }
    ...
    let sentDate = '2025-12-15'; // 기본값 (263번 발송일 기준)
    if (message.sent_at) {
    ...
    const { error: uploadError } = await supabase.storage
      ...
      .upload(storagePath, imageBuffer, {
        contentType: fileExt.toLowerCase() === '.png' ? 'image/png' : 'image/jpeg',
        upsert: true
      });
    ...
    if (metaError) {
      console.error('❌ 메타데이터 upsert 실패:', metaError.message);
      process.exit(1);
    }

    console.log(`✅ 메타데이터 저장/업데이트 성공 (ID: ${metadata.id})\n`);

    // 7. channel_sms.image_url 업데이트 (이제 Supabase URL을 직접 저장)
    ...
    console.log(`   메타데이터 ID: ${metadata.id}`);
    console.log(`   파일 크기: ${(fileSize / 1024).toFixed(2)}KB\n`);
    console.log('💡 SMS 편집 페이지에서 확인하세요: /admin/sms?id=263');
...
recoverMessage263ImageFromFile(imageFilePath)
  .then(() => {
    console.log('✅ 완료!');
    process.exit(0);
  })
