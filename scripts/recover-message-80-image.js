/**
 * 80번 메시지의 이미지 복구 스크립트
 * 솔라피에서 이미지를 다운로드하여 우리 서버에 저장
 * 
 * 사용법:
 * node scripts/recover-message-80-image.js
 */
...
async function recoverMessage80Image() {
  console.log('🔄 80번 메시지 이미지 복구 시작...\n');
  ...
  try {
    ...
    // RAW DATA 전체 출력 (디버깅용)
    console.log('📋 RAW DATA (메시지 목록):');
    console.log(JSON.stringify(messageListData, null, 2));
    console.log('');

    // 3. 이미지 ID 추출 (messageList 객체에서 추출)
    let imageId = null;
    ...
    if (messageListData.messageList) {
      ...
      if (messageKeys.length > 0) {
        ...
        // RAW DATA에서 imageId 추출 (다양한 필드명 시도)
        ...
        if (imageId) {
          ...
          console.log('   메시지 RAW DATA:', JSON.stringify(firstMessage, null, 2));
          console.log('   사용 가능한 키:', Object.keys(firstMessage));
        }
      }
    } else if (messageListData.messages && messageListData.messages.length > 0) {
      // 대안: messages 배열 형태 (구버전 API 응답)
    ...
    if (!imageId && message.image_url && !message.image_url.startsWith('http')) {
      // Solapi imageId는 일반적으로 특정 형식 (예: ST01...)
    ...
    if (!imageId) {
      ...
      console.log('   사용법: node scripts/recover-message-80-image.js <imageId>');
      process.exit(1);
    }
    ...
    // 방법 1: Storage API 다운로드 엔드포인트 시도
    let imageBuffer = null;
    let downloadSuccess = false;

    // 솔라피 Storage 다운로드 URL (문서에 따라 다를 수 있음)
    const downloadUrls = [
    ...
    // 방법 2: 다운로드 실패 시 기존 Supabase URL 확인
    if (!downloadSuccess) {
      console.error('❌ 솔라피 Storage API로 이미지 다운로드 실패');
      
      // 기존 Supabase URL이 있는지 확인
      if (message.image_url && message.image_url.startsWith('http')) {
        console.log(`\n📋 기존 Supabase URL 발견: ${message.image_url}`);
        console.log('   이 URL에서 이미지를 확인 중...\n');
        
        try {
          const existingImageResponse = await fetch(message.image_url);
          if (existingImageResponse.ok) {
            const existingImageBuffer = await existingImageResponse.arrayBuffer();
            const existingImageSize = existingImageBuffer.byteLength;
            
            console.log(`   기존 이미지 크기: ${(existingImageSize / 1024).toFixed(2)}KB`);
            
            if (existingImageSize > 10 * 1024) {
              // 10KB 이상이면 실제 이미지로 간주
              console.log('   ✅ 기존 이미지가 유효합니다. 이미 복구되어 있습니다.\n');
              console.log(`💡 SMS 편집 페이지에서 확인하세요: /admin/sms?id=${messageId}\n`);
              process.exit(0);
            } else {
              console.log('   ⚠️ 기존 이미지가 너무 작습니다. 실제 이미지가 아닐 수 있습니다.\n');
            }
          }
        } catch (e) {
          console.log(`   ⚠️ 기존 URL 접근 실패: ${e.message}\n`);
        }
      }
      
      console.log('\n💡 대안 방법:');
      console.log('   1. 솔라피 콘솔에서 이미지를 직접 다운로드');
      console.log('      - https://console.solapi.com/message-log 에서 80번 메시지의 그룹 ID로 검색');
      console.log('      - 메시지 상세에서 이미지를 다운로드');
      console.log('      - 다운로드한 이미지를 SMS 편집 페이지에서 업로드');
      console.log('   2. 또는 수동으로 imageId를 Supabase에 저장 (임시)');
      console.log(`\n   현재 imageId: ${imageId}`);
      console.log(`   솔라피 그룹 ID: ${groupId}`);
      console.log(`   솔라피 메시지 ID: ${messageListData.messageList ? Object.keys(messageListData.messageList)[0] : '없음'}\n`);
      
      // imageId를 임시로 저장 (나중에 수동으로 교체 가능)
      const { error: updateError } = await supabase
        .from('channel_sms')
        .update({
          image_url: imageId, // 임시 저장
      ...
      console.log('✅ channel_sms.image_url 업데이트 완료 (임시 - Solapi imageId)');
      console.log('💡 솔라피 콘솔에서 이미지를 다운로드하여 SMS 편집 페이지에서 재업로드하세요.\n');
      process.exit(0);
    }

    // 6. Supabase Storage에 저장
    const dateFolder = sentDate; // 2025-11-18
    ...
    console.log('💡 SMS 편집 페이지에서 확인하세요: /admin/sms?id=80\n');
...
// 명령줄 인자로 imageId를 받을 수 있음
const args = process.argv.slice(2);
if (args.length > 0) {
  const manualImageId = args[0];
  console.log(`📌 수동으로 이미지 ID 입력: ${manualImageId}\n`);
  // 수동 imageId를 사용하도록 수정 필요 (현재는 자동 감지 우선)
}

recoverMessage80Image()
  .then(() => {
    console.log('✅ 완료!');
    process.exit(0);
  })
