...
async function verifyImages(url, pagePath) {
  ...
  try {
    ...
    // picture 태그 확인
    const pictures = await page.$$eval('picture', pics => 
      pics.map(pic => {
    ...
    if (pictures.length > 0) {
      console.log(`  📸 picture 태그: ${pictures.length}개`);
      pictures.forEach((pic, idx) => {
        console.log(`    ${idx + 1}. source: ${pic.sourceSrcset} (${pic.sourceType})`);
        console.log(`       img: ${pic.imgSrc}`);
      });
    }
    ...
    return { total: 0, broken: 0, loaded: 0, brokenImages: [], pictures: 0, error: error.message };
  } finally {
    await page.close();
    await browser.close();
  }
...
async function main() {
  const targetUrl = 'https://mas-4jqutjgpu-taksoo-kims-projects.vercel.app';
  ...
  results.forEach(result => {
    ...
    console.log(`  picture 태그: ${result.pictures}개`);
    
    totalImages += result.total;
    totalBroken += result.broken;
    totalLoaded += result.loaded;
    totalPictures += result.pictures;
  });
  ...
  console.log(`  picture 태그: ${totalPictures}개`);
  console.log(`${'='.repeat(60)}`);
