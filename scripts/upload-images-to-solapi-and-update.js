/**
 * 메시지 140-147의 이미지를 Solapi에 업로드하고 imageId로 업데이트
 * 배포된 환경의 API를 사용하여 Sharp 모듈 문제 회피
 */
...
async function uploadImagesAndUpdate() {
  ...
  console.log('🖼️  메시지 140-147 이미지 Solapi 업로드 및 업데이트');
  console.log('='.repeat(80));
  console.log('');
  ...
  for (const msg of messages) {
    ...
    try {
      ...
      if (!reuploadResponse.ok) {
        ...
        throw new Error(`HTTP ${reuploadResponse.status}: ${errorText.substring(0, 200)}`);
      }
  ...
  console.log('\n' + '='.repeat(80));
  console.log('✅ 완료! 이제 메시지를 재발송할 수 있습니다.');
}

uploadImagesAndUpdate().catch(console.error);
