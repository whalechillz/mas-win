/**
 * 내부자 "25730905" 예약 및 고객 삭제 스크립트
 * 
 * 전화번호나 이름에 "25730905"가 포함된 모든 예약과 고객을 삭제합니다.
 * 
 * 사용법:
 * node scripts/delete-internal-25730905.js [--dry-run]
 * 
 * 옵션:
...
async function deleteInternal25730905() {
  const dryRun = process.argv.includes('--dry-run');
  const searchTerm = '25730905';
  
  console.log(`🔍 "${searchTerm}" 포함 예약 및 고객 조회 중...\n`);
  if (dryRun) {
  ...
  if (bookingCount === 0 && customerCount === 0) {
    console.log(`✅ "${searchTerm}"가 포함된 예약이나 고객이 없습니다.\n`);
    return;
  }
