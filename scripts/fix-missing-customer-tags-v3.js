/**
 * 마이그레이션 누락된 ai_tags 복구 스크립트 (v3)
 * 
 * 문제: file_path에는 있지만 ai_tags에 customer-{id} 태그가 없는 이미지들
...
async function fixMissingCustomerTags() {
  ...
  // 2. file_path에는 있지만 ai_tags에 customer-{id}가 없는 이미지 조회
  ...
  if (pathError) {
    console.error('❌ 이미지 조회 오류:', pathError);
    return;
  }
  
  console.log(`✅ ${imagesByPath?.length || 0}개의 이미지 발견\n`);
  ...
  for (const img of imagesToUpdate) {
    try {
      ...
      if (updateError) {
        ...
        console.log(`✅ [${img.id}] ${img.folderName} -> customer-${img.customerId} 태그 추가`);
        successCount++;
      }
    } catch (error) {
