 ...
 * @returns {Promise<boolean>} 유효한 URL이면 true
 */
async function validateImageUrl(imageUrl) {
  ...
  try {
    ...
    // 200-299 범위의 상태 코드면 유효
    return response.ok;
  } catch (error) {
    // 네트워크 에러나 타임아웃 등
    console.warn(`⚠️ 이미지 URL 검증 실패: ${imageUrl}`, error.message);
    return false;
  }
}
 ...
 * @returns {Promise<Object>} 슬랙 메시지 객체 (Block Kit 형식)
 */
export async function formatKakaoContentSlackMessage({ 
  date, 
  account1Data, 
  account2Data, 
  feedData, 
  ...
  if (account1Data && (account1Data.created || includeNotCreated)) {
    ...
    if (account1Data.background?.imageUrl) {
      const isValidImage = await validateImageUrl(account1Data.background.imageUrl);
      if (isValidImage) {
        blocks.push({
          type: 'section',
          text: {
    ...
    if (account1Data.profile?.imageUrl) {
      const isValidImage = await validateImageUrl(account1Data.profile.imageUrl);
      if (isValidImage) {
        blocks.push({
          type: 'section',
          text: {
    ...
    if (account1Data.message) {
      ...
      // 코드 블록 (복사용) - 각 줄을 별도로 표시하여 선택 용이
      const messageLines = account1Data.message.split('\n');
    ...
    if (feedData?.account1?.imageUrl) {
      const isValidImage = await validateImageUrl(feedData.account1.imageUrl);
      if (isValidImage) {
        blocks.push({
          type: 'section',
          text: {
    ...
    if (feedData?.account1?.caption) {
      ...
      // 코드 블록 (복사용) - 각 줄을 별도로 표시하여 선택 용이
      const captionLines = feedData.account1.caption.split('\n');
  ...
  if (account2Data && (account2Data.created || includeNotCreated)) {
    ...
    if (account2Data.background?.imageUrl) {
      const isValidImage = await validateImageUrl(account2Data.background.imageUrl);
      if (isValidImage) {
        blocks.push({
          type: 'section',
          text: {
    ...
    if (account2Data.profile?.imageUrl) {
      const isValidImage = await validateImageUrl(account2Data.profile.imageUrl);
      if (isValidImage) {
        blocks.push({
          type: 'section',
          text: {
    ...
    if (account2Data.message) {
      ...
      // 코드 블록 (복사용) - 각 줄을 별도로 표시하여 선택 용이
      const messageLines = account2Data.message.split('\n');
      messageLines.forEach((line) => {
        if (line.trim()) {
          blocks.push({
            type: 'section',
    ...
    if (feedData?.account2?.imageUrl) {
      const isValidImage = await validateImageUrl(feedData.account2.imageUrl);
      if (isValidImage) {
        blocks.push({
          type: 'section',
          text: {
    ...
    if (feedData?.account2?.caption) {
      ...
      // 코드 블록 (복사용) - 각 줄을 별도로 표시하여 선택 용이
      const captionLines = feedData.account2.caption.split('\n');
      captionLines.forEach((line) => {
        if (line.trim()) {
          blocks.push({
            type: 'section',
