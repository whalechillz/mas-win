...
export async function compressImageForSolapi(imageBuffer, maxSize = 200 * 1024) {
  let sharp;
  try {
    // Vercel 환경에서 명시적으로 linux-x64 바이너리 사용 시도
    if (process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME) {
      try {
        // 먼저 플랫폼별 바이너리 직접 import 시도
        const sharpLinux = await import('@img/sharp-linux-x64');
        sharp = sharpLinux.default;
        console.log('✅ @img/sharp-linux-x64 직접 로드 성공');
      } catch (linuxError) {
        console.warn('⚠️ @img/sharp-linux-x64 직접 로드 실패, sharp로 fallback:', linuxError.message);
        // fallback to regular sharp
        sharp = (await import('sharp')).default;
      }
    } else {
      // 로컬 환경에서는 일반 sharp 사용
      sharp = (await import('sharp')).default;
    }
  } catch (importError) {
