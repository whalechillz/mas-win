...
export async function compressImageForSolapi(imageBuffer, maxSize = 200 * 1024) {
  let sharp;
  try {
    // sharp는 자동으로 플랫폼에 맞는 바이너리를 선택합니다
    // @img/sharp-linux-x64는 직접 import할 수 없고, sharp가 자동으로 사용합니다
    // dependencies에 @img/sharp-linux-x64가 포함되어 있으므로 sharp가 자동으로 사용
    const sharpModule = await import('sharp');
    sharp = sharpModule.default;
    
    // Sharp가 제대로 로드되었는지 확인
    if (!sharp) {
      throw new Error('Sharp 모듈이 undefined입니다.');
    }
  } catch (importError) {
