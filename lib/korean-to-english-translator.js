...
/**
 * 단일 한글 문자를 로마자로 변환 (표준 로마자 표기법)
 * @param {string} char - 변환할 한글 문자
 * @returns {string} - 로마자로 변환된 문자
 */
function romanizeSingleChar(char) {
  ...
  // 표준 로마자 표기법 매핑 (Revised Romanization of Korean)
  const initialConsonants = ['g', 'kk', 'n', 'd', 'tt', 'r', 'm', 'b', 'pp', 's', 'ss', '', 'j', 'jj', 'ch', 'k', 't', 'p', 'h'];
  const vowels = ['a', 'ae', 'ya', 'yae', 'eo', 'e', 'yeo', 'ye', 'o', 'wa', 'wae', 'oe', 'yo', 'u', 'wo', 'we', 'wi', 'yu', 'eu', 'ui', 'i'];
  const finalConsonants = ['', 'k', 'k', 'k', 'n', 'n', 'n', 'n', 't', 'l', 'k', 'm', 'l', 'l', 'l', 'p', 'l', 'm', 'p', 'p', 't', 't', 'ng', 't', 't', 'k', 't', 'p', 't'];
  
  let romanized = '';
  if (initial < initialConsonants.length) {
    romanized += initialConsonants[initial];
  }
  if (vowel < vowels.length) {
    romanized += vowels[vowel];
  }
...
function translateKoreanToEnglish(text) {
  ...
  // 남은 한글 문자가 있으면 로마자 변환 시도
  if (/[가-힣]/.test(result)) {
    // 표준 한국 성씨 로마자 표기법 (관습적 표기 우선)
    const koreanSurnames = {
      '김': 'kim',
      '이': 'lee',      // I가 아니라 Lee (관습적)
      '박': 'park',     // Bak이 아니라 Park (관습적)
      '최': 'choi',
      '정': 'jung',     // Jeong이 아니라 Jung (관습적)
      '강': 'kang',
      '조': 'cho',      // Jo가 아니라 Cho (관습적)
      '윤': 'yoon',
      '장': 'jang',
      '임': 'lim',
      '한': 'han',
    ...
    const fullNameMapping = {
      ...
      '박정원': 'parkjeongwon',
      '박성원': 'parkseongwon',
      '박중진': 'parkjungjin',  // 표준 로마자 표기법 적용
      '이수원': 'leesuwon',     // 표준 로마자 표기법 적용
      '김섬묵': 'gimseommuk',
      '원종문': 'wonjongmun',
      '신희갑': 'shinhuigap',
      '조양현': 'joyanghyeon',
    ...
    // 전체 이름 직접 매핑 우선 적용
    if (fullNameMapping[result]) {
      result = fullNameMapping[result];
    } else {
      // 이름이 2-3글자 한글인 경우 (고객 이름으로 추정)
      const koreanNamePattern = /^[가-힣]{2,3}$/;
      if (koreanNamePattern.test(result)) {
        const surname = result.charAt(0);
        const givenName = result.substring(1);
        
        // 성씨를 표준 로마자 표기법으로 변환
        let surnameRoman = koreanSurnames[surname];
        if (!surnameRoman) {
          // 성씨 매핑에 없으면 일반 로마자 변환
          surnameRoman = romanizeSingleChar(surname);
        }
        
        // 이름 부분을 로마자로 변환
        let givenNameRoman = '';
        for (let i = 0; i < givenName.length; i++) {
          givenNameRoman += romanizeSingleChar(givenName.charAt(i));
        }
        
        result = surnameRoman + givenNameRoman;
      } else {
        // 일반 텍스트인 경우 문자별로 변환
        result = result.replace(/[가-힣]/g, (char) => {
          return romanizeSingleChar(char);
        });
      }
    }
  }
