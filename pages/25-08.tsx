...
export default function Funnel202508() {
  ...
  const [assignedVersion] = useState<string>("B"); // 항상 B 버전 (승자)
    ...
    if (isClient) {
      const sendGA4Events = () => {
        if (typeof window !== 'undefined' && (window as any).gtag) {
          ...
          console.log('A/B 테스트 이벤트 설정 완료: 버전 B (승자)');
        } else {
  ...
  return (
    <>
      ...
      {/* 승자 버전 (B) iframe */}
      <iframe
        src="/versions/funnel-2025-08-live-b.html"
        style={{
          width: '100%',
          height: '100vh',
          border: 'none',
