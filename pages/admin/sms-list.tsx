...
export default function SMSListAdmin() {
  ...
  // 이미지 소스 배지 함수
  const getImageSourceBadge = (imageUrl?: string) => {
    if (!imageUrl) {
      return (
        <span className="px-1.5 py-0.5 text-[10px] font-semibold rounded bg-gray-100 text-gray-600 border border-gray-300">
          없음
        </span>
      );
    }
    
    if (imageUrl.startsWith('ST01FZ')) {
      return (
        <span className="px-1.5 py-0.5 text-[10px] font-semibold rounded bg-blue-100 text-blue-700 border border-blue-300" title="Solapi Storage">
          Solapi
        </span>
      );
    }
    ...
    if (imageUrl.includes('supabase.co')) {
      return (
        <span className="px-1.5 py-0.5 text-[10px] font-semibold rounded bg-green-100 text-green-700 border border-green-300" title="Supabase Storage">
          Supabase
        </span>
      );
    }
                        ...
                        <td className="px-2 py-2 text-center">
                          {message.message_type === 'MMS' && message.image_url ? (
                            <div className="flex flex-col items-center gap-1">
                              <div className="relative w-10 h-10 bg-gray-100 rounded overflow-hidden border border-gray-300">
                                <img
                                  src={message.image_url.startsWith('ST01FZ') 
                                    ? `/api/solapi/get-image-preview?imageId=${message.image_url}&messageId=${message.id}`
                                    : message.image_url
                                  }
                                  alt="MMS 이미지"
                              ...
                              {getImageSourceBadge(message.image_url)}
                            </div>
                          ) : (
