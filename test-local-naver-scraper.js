...
async function testLocalNaverScraper() {
  console.log('🚀 로컬 네이버 블로그 스크래퍼 테스트 시작...');
  ...
  try {
    ...
    try {
      ...
      console.log('✅ 로컬 스크래핑 결과 섹션이 표시되었습니다.');
      ...
      console.log('⚠️ 로컬 스크래핑 결과가 아직 표시되지 않았습니다.');
    }
    ...
    try {
      ...
      console.log('✅ 로컬 URL 모드 스크래핑도 성공적으로 완료되었습니다.');
    } catch (error) {
      console.log('⚠️ 로컬 URL 모드 스크래핑 결과가 아직 표시되지 않았습니다.');
    }
    
    console.log('\n🎉 로컬 네이버 블로그 스크래퍼 테스트 완료!');
    
  } catch (error) {
    console.error('❌ 로컬 테스트 중 오류 발생:', error);
  } finally {
    await browser.close();
  }
}

testLocalNaverScraper().catch(console.error);
